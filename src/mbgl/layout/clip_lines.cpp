    ...
    for (auto& line : lines) {
        ...
        for (auto it = line.begin(); it != end; it++) {
            ...
            if (p0.x < x1 && p1.x < x1) {
                ...
                p0 = { x1, static_cast<int16_t>(::round(
                               p0.y + (p1.y - p0.y) * ((float)(x1 - p0.x) / (p1.x - p0.x)))) };
            } else if (p1.x < x1) {
                p1 = { x1, static_cast<int16_t>(::round(
                               p0.y + (p1.y - p0.y) * ((float)(x1 - p0.x) / (p1.x - p0.x)))) };
            }
            ...
            if (p0.y < y1 && p1.y < y1) {
                ...
                p0 = { static_cast<int16_t>(
                           ::round(p0.x + (p1.x - p0.x) * ((float)(y1 - p0.y) / (p1.y - p0.y)))),
                       y1 };
            } else if (p1.y < y1) {
            ...
            if (p0.x >= x2 && p1.x >= x2) {
                ...
                p0 = { x2, static_cast<int16_t>(::round(
                               p0.y + (p1.y - p0.y) * ((float)(x2 - p0.x) / (p1.x - p0.x)))) };
            } else if (p1.x >= x2) {
                p1 = { x2, static_cast<int16_t>(::round(
                               p0.y + (p1.y - p0.y) * ((float)(x2 - p0.x) / (p1.x - p0.x)))) };
            }
            ...
            if (p0.y >= y2 && p1.y >= y2) {
                ...
                p0 = { static_cast<int16_t>(
                           ::round(p0.x + (p1.x - p0.x) * ((float)(y2 - p0.y) / (p1.y - p0.y)))),
                       y2 };
            } else if (p1.y >= y2) {
                p1 = { static_cast<int16_t>(
                           ::round(p0.x + (p1.x - p0.x) * ((float)(y2 - p0.y) / (p1.y - p0.y)))),
            ...
            if (clippedLines.empty() ||
                (!clippedLines.back().empty() && !(p0 == clippedLines.back().back()))) {
