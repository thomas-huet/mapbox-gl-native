 ...
 * Like the above, but rotated so axis of the space are aligned with the viewport instead of the
 * tile.
 * This space is used for pitch-alignment=map, rotation-alignment=viewport
 *
 * ## Viewport pixel coordinate space
 * (0, 0) is at the top left of the canvas and (pixelWidth, pixelHeight) is at the bottom right
 * corner
 ...
 * 2. if the label follows a line, find the point along the line that is the correct distance from
 * the anchor.
 ...
 * For labels aligned with lines we have to steps 1 and 2 on the cpu since we need access to the
 * line geometry.
 *      This is what `updateLineLabels(...)` does.
 *      Since the conversion is handled on the cpu we just set `u_label_plane_matrix` to an identity
 * matrix.
 *
...
float evaluateSizeForFeature(const ZoomEvaluatedSize& zoomEvaluatedSize,
                             const PlacedSymbol& placedSymbol) {
    if (zoomEvaluatedSize.isFeatureConstant) {
        ...
        if (zoomEvaluatedSize.isZoomConstant) {
            ...
            return placedSymbol.lowerSize +
                   zoomEvaluatedSize.sizeT * (placedSymbol.upperSize - placedSymbol.lowerSize);
        }
    }
}
...
bool isVisible(const vec4& anchorPos, const std::array<double, 2>& clippingBuffer) {
    ...
    const bool inPaddedViewport = (x >= -clippingBuffer[0] && x <= clippingBuffer[0] &&
                                   y >= -clippingBuffer[1] && y <= clippingBuffer[1]);
...
void hideGlyphs(size_t numGlyphs,
                gl::VertexVector<SymbolDynamicLayoutAttributes::Vertex>& dynamicVertexArray) {
...
enum PlacementResult { OK, NotEnoughRoom, NeedsFlipping, UseVertical };
...
Point<float> projectTruncatedLineSegment(const Point<float>& previousTilePoint,
    ...
    // We are assuming "previousTilePoint" won't project to a point within one unit of the camera
    // plane
    // If it did, that would mean our label extended all the way out from within the viewport to a
    // (very distant)
    // point near the plane of the camera. We wouldn't be able to render the label anyway once it
    // crossed the
    // plane of the camera.
    const Point<float> projectedUnitVertex =
        project(previousTilePoint + util::unit<float>(previousTilePoint - currentTilePoint),
                projectionMatrix)
            .first;
    const Point<float> projectedUnitSegment = previousProjectedPoint - projectedUnitVertex;

    return previousProjectedPoint +
           (projectedUnitSegment * (minimumLength / util::mag<float>(projectedUnitSegment)));
}

optional<PlacedGlyph> placeGlyphAlongLine(const float offsetX,
    ...
    while (distanceToPrev + currentSegmentDistance <= absOffsetX) {
        ...
        prev = current;
        PointAndCameraDistance projection =
            project(convertPoint<float>(line.at(currentIndex)), labelPlaneMatrix);
        if (projection.second > 0) {
            ...
            const Point<float> previousTilePoint =
                distanceToPrev == 0 ? tileAnchorPoint
                                    : convertPoint<float>(line.at(currentIndex - dir));
            const Point<float> currentTilePoint = convertPoint<float>(line.at(currentIndex));
            current =
                projectTruncatedLineSegment(previousTilePoint, currentTilePoint, prev,
                                            absOffsetX - distanceToPrev + 1, labelPlaneMatrix);
        }
    ...
    p += util::perp(prevToCurrent) *
         static_cast<float>(lineOffsetY * dir / util::mag(prevToCurrent));

    const float segmentAngle = angle + std::atan2(current.y - prev.y, current.x - prev.x);

    return { { p, segmentAngle, returnTileDistance
                                    ? TileDistance((currentIndex - dir) == initialIndex
                                                       ? 0
                                                       : tileDistances[currentIndex - dir],
                                                   absOffsetX - distanceToPrev)
                                    : optional<TileDistance>() } };
}

optional<std::pair<PlacedGlyph, PlacedGlyph>>
placeFirstAndLastGlyph(const float fontScale,
                       const float lineOffsetX,
                       const float lineOffsetY,
                       const bool flip,
                       const Point<float>& anchorPoint,
    ...
    optional<PlacedGlyph> firstPlacedGlyph = placeGlyphAlongLine(
        fontScale * firstGlyphOffset, lineOffsetX, lineOffsetY, flip, anchorPoint, tileAnchorPoint,
        symbol.segment, symbol.line, symbol.tileDistances, labelPlaneMatrix, returnTileDistance);
    if (!firstPlacedGlyph)
        return optional<std::pair<PlacedGlyph, PlacedGlyph>>();

    optional<PlacedGlyph> lastPlacedGlyph = placeGlyphAlongLine(
        fontScale * lastGlyphOffset, lineOffsetX, lineOffsetY, flip, anchorPoint, tileAnchorPoint,
        symbol.segment, symbol.line, symbol.tileDistances, labelPlaneMatrix, returnTileDistance);
    if (!lastPlacedGlyph)
        return optional<std::pair<PlacedGlyph, PlacedGlyph>>();
...
optional<PlacementResult> requiresOrientationChange(const WritingModeType writingModes,
    ...
    if (writingModes == (WritingModeType::Horizontal | WritingModeType::Vertical)) {
        // On top of choosing whether to flip, choose whether to render this version of the glyphs
        // or the alternate
        // vertical glyphs. We can't just filter out vertical glyphs in the horizontal range because
        // the horizontal
        // and vertical versions can have slightly different projections which could lead to angles
        // where both or
        // neither showed.
    ...
    if ((writingModes == WritingModeType::Vertical) ? (firstPoint.y < lastPoint.y)
...
PlacementResult
placeGlyphsAlongLine(const PlacedSymbol& symbol,
    ...
    if (symbol.glyphOffsets.size() > 1) {
        ...
        const optional<std::pair<PlacedGlyph, PlacedGlyph>> firstAndLastGlyph =
            placeFirstAndLastGlyph(fontScale, lineOffsetX, lineOffsetY, flip, projectedAnchorPoint,
                                   symbol.anchorPoint, symbol, labelPlaneMatrix, false);
        if (!firstAndLastGlyph) {
            return PlacementResult::NotEnoughRoom;
        }

        const Point<float> firstPoint =
            project(firstAndLastGlyph->first.point, glCoordMatrix).first;
        const Point<float> lastPoint =
            project(firstAndLastGlyph->second.point, glCoordMatrix).first;

        if (keepUpright && !flip) {
            auto orientationChange =
                requiresOrientationChange(symbol.writingModes, firstPoint, lastPoint, aspectRatio);
        ...
        for (size_t glyphIndex = 1; glyphIndex < symbol.glyphOffsets.size() - 1; glyphIndex++) {
            ...
            // Since first and last glyph fit on the line, we're sure that the rest of the glyphs
            // can be placed
            auto placedGlyph =
                placeGlyphAlongLine(glyphOffsetX * fontScale, lineOffsetX, lineOffsetY, flip,
                                    projectedAnchorPoint, symbol.anchorPoint, symbol.segment,
        ...
        if (keepUpright && !flip) {
            ...
            const Point<float> tileSegmentEnd =
                convertPoint<float>(symbol.line.at(symbol.segment + 1));
            const PointAndCameraDistance projectedVertex = project(tileSegmentEnd, posMatrix);
            // We know the anchor will be in the viewport, but the end of the line segment may be
            // behind the plane of the camera, in which case we can use a point at any arbitrary
            // (closer)
            ...
            auto orientationChange =
                requiresOrientationChange(symbol.writingModes, a, b, aspectRatio);
            if (orientationChange) {
                return *orientationChange;
            }
        }
        const float glyphOffsetX = symbol.glyphOffsets.front();
        optional<PlacedGlyph> singleGlyph =
            placeGlyphAlongLine(fontScale * glyphOffsetX, lineOffsetX, lineOffsetY, flip,
                                projectedAnchorPoint, symbol.anchorPoint, symbol.segment,
                                symbol.line, symbol.tileDistances, labelPlaneMatrix, false);
        if (!singleGlyph)
            return PlacementResult::NotEnoughRoom;
    ...
    // The number of placedGlyphs must equal the number of glyphOffsets, which must correspond to
    // the number of glyph vertices
...
void reprojectLineLabels(
    ...
    const std::array<double, 2> clippingBuffer = { { 256.0 / state.getSize().width * 2.0 + 1.0,
                                                     256.0 / state.getSize().height * 2.0 + 1.0 } };
    ...
    const mat4 labelPlaneMatrix =
        getLabelPlaneMatrix(posMatrix, pitchWithMap, rotateWithMap, state, pixelsToTileUnits);

    const mat4 glCoordMatrix =
        getGlCoordMatrix(posMatrix, pitchWithMap, rotateWithMap, state, pixelsToTileUnits);
    ...
    for (auto& placedSymbol : placedSymbols) {
        ...
        if (placedSymbol.hidden ||
            (placedSymbol.writingModes == WritingModeType::Vertical && !useVertical)) {
            hideGlyphs(placedSymbol.glyphOffsets.size(), dynamicVertexArray);
            continue;
        }
        // Awkward... but we're counting on the paired "vertical" symbol coming immediately after
        ...
        const float perspectiveRatio =
            0.5 + 0.5 * (cameraToAnchorDistance / state.getCameraToCenterDistance());
        ...
        PlacementResult placeUnflipped =
            placeGlyphsAlongLine(placedSymbol, pitchScaledFontSize, false /*unflipped*/,
                                 values.keepUpright, posMatrix, labelPlaneMatrix, glCoordMatrix,
                                 dynamicVertexArray, anchorPoint, state.getSize().aspectRatio());
        ...
        if (placeUnflipped == PlacementResult::NotEnoughRoom || useVertical ||
            (placeUnflipped == PlacementResult::NeedsFlipping &&
             placeGlyphsAlongLine(placedSymbol, pitchScaledFontSize, true /*flipped*/,
                                  values.keepUpright, posMatrix, labelPlaneMatrix, glCoordMatrix,
                                  dynamicVertexArray, anchorPoint, state.getSize().aspectRatio()) ==
                 PlacementResult::NotEnoughRoom)) {
            hideGlyphs(placedSymbol.glyphOffsets.size(), dynamicVertexArray);
        }
    }
}
