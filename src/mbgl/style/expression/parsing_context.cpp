...
ParseResult ParsingContext::parse(const Convertible& value,
    ...
    ParsingContext child(key + "[" + util::toString(index_) + "]", errors, std::move(expected_),
                         scope);
    return child.parse(value, typeAnnotationOption);
}

ParseResult
ParsingContext::parse(const Convertible& value,
                      std::size_t index_,
                      optional<type::Type> expected_,
...
const ExpressionRegistry& getExpressionRegistry() {
    static ExpressionRegistry registry{ { { "==", Equals::parse },
                                          { "!=", Equals::parse },
                                          { "all", All::parse },
                                          { "any", Any::parse },
...
ParseResult ParsingContext::parse(const Convertible& value,
    ...
    if (isArray(value)) {
        ...
        if (!op) {
            error("Expression name must be a string, but found " +
    ...
    if (expected) {
        ...
        if ((*expected == type::String || *expected == type::Number || *expected == type::Boolean ||
            ...
            if (typeAnnotationOption == includeTypeAnnotations) {
                parsed = { std::make_unique<ArrayAssertion>(expected->get<type::Array>(),
                                                            std::move(*parsed)) };
            }
    ...
    if (!dynamic_cast<Literal*>(parsed->get()) && isConstant(**parsed)) {
        ...
        if (type.is<type::Array>()) {
            ...
            return ParseResult(std::make_unique<Literal>(type.get<type::Array>(),
...
ParsingContext::parseLayerPropertyExpression(const Convertible& value,
    ...
    if (parsed && !isZoomConstant(**parsed)) {
        optional<variant<const InterpolateBase*, const Step*, ParsingError>> zoomCurve =
            findZoomCurve(parsed->get());
        if (!zoomCurve) {
            error(
