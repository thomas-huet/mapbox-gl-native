...
ParseResult Let::parse(const Convertible& value, ParsingContext& ctx) {
    ...
    if (length < 4) {
        ctx.error("Expected at least 3 arguments, but found " + util::toString(length - 1) +
                  " instead.");
        return ParseResult();
    }
    ...
    for (std::size_t i = 1; i < length - 1; i += 2) {
        ...
        if (!name) {
            ctx.error("Expected string, but found " + getJSONType(arrayMember(value, i)) +
                          " instead.",
                      i);
            return ParseResult();
        }

        bool isValidName = std::all_of(name->begin(), name->end(),
    ...
    ParseResult result_ =
        ctx.parse(arrayMember(value, length - 1), length - 1, ctx.getExpected(), bindings_);
    if (!result_) {
        return ParseResult();
    }
