...
EvaluationResult ArrayAssertion::evaluate(const EvaluationContext& params) const {
    ...
    if (checkSubtype(expected, actual)) {
        return EvaluationError{ "Expected value to be of type " + toString(expected) +
...
ParseResult ArrayAssertion::parse(const Convertible& value, ParsingContext& ctx) {
    ...
    static std::unordered_map<std::string, type::Type> itemTypes{ { "string", type::String },
                                                                  { "number", type::Number },
                                                                  { "boolean", type::Boolean } };

    auto length = arrayLength(value);
    if (length < 2 || length > 4) {
        ctx.error("Expected 1, 2, or 3 arguments, but found " + util::toString(length - 1) +
                  " instead.");
        return ParseResult();
    }
    ...
    if (length > 2) {
        ...
        if (it == itemTypes.end()) {
            ctx.error(
                R"(The item type argument of "array" must be one of string, number, boolean)", 1);
            return ParseResult();
        }
        itemType = it->second;
    } else {
    ...
    if (length > 3) {
        ...
        if (!n || *n != std::floor(*n)) {
            ctx.error(
                R"(The length argument to "array" must be a positive integer literal.)", 2);
            return ParseResult();
        }
    ...
    return ParseResult(
        std::make_unique<ArrayAssertion>(type::Array(*itemType, N), std::move(*input)));
}
