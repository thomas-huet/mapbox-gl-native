...
optional<Value> parseValue(const Convertible& value, ParsingContext& ctx) {
    ...
    if (isObject(value)) {
        ...
        bool error = false;
        eachMember(value, [&](const std::string& k, const mbgl::style::conversion::Convertible& v)
    ...
    return v->match([&](uint64_t n) { return checkNumber(n); },
                    ...
                    [&](const auto&) { return optional<Value>(toExpressionValue(*v)); });
}
...
ParseResult Literal::parse(const Convertible& value, ParsingContext& ctx) {
    if (isObject(value)) {
        ...
        if (arrayLength(value) != 2) {
            ctx.error("'literal' expression requires exactly one argument, but found " +
        ...
        if (ctx.getExpected() && ctx.getExpected()->template is<type::Array>() &&
            ...
            if (type.N && (*type.N == 0) && (!expected.N || (*expected.N == 0))) {
                return ParseResult(std::make_unique<Literal>(
                    expected, parsedValue->template get<std::vector<Value>>()));
            }
        }
...
mbgl::Value Literal::serialize() const {
    if (getType().is<type::Array>() || getType().is<type::ObjectType>()) {
        return std::vector<mbgl::Value>{ { getOperator(),
                                           *fromExpressionValue<mbgl::Value>(value) } };
    } else {
