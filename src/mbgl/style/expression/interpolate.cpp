...
ParseResult parseInterpolate(const Convertible& value, ParsingContext& ctx) {
    ...
    if (interpName && *interpName == "linear") {
        ...
        if (!x1 || !y1 || !x2 || !y2 || *x1 < 0 || *x1 > 1 || *y1 < 0 || *y1 > 1 || *x2 < 0 ||
            *x2 > 1 || *y2 < 0 || *y2 > 1) {
            ctx.error("Cubic bezier interpolation requires four numeric arguments with values "
                      "between 0 and 1.",
                      1);
            return ParseResult();
        }
    ...
    if (*outputType != type::Number && *outputType != type::Color &&
    ...
    return outputType->match(
        ...
        [&](const type::Array& arrayType) -> ParseResult {
            return interpolator->match([&](const auto& continuousInterpolator) {
...
mbgl::Value Interpolate<T>::serialize() const {
    ...
    interpolator.match(
        [&](const ExponentialInterpolator& exponential) {
            if (exponential.base == 1) {
                ...
                serialized.emplace_back(
                    std::vector<mbgl::Value>{ { std::string("exponential"), exponential.base } });
            }
        },
        [&](const CubicBezierInterpolator& cubicBezier) {
            ...
            serialized.emplace_back(std::vector<mbgl::Value>{
                { cubicBezierTag, p1.first, p1.second, p2.first, p2.second } });
        });
