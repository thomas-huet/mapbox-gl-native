...
void CustomTileLoader::fetchTile(const OverscaledTileID& tileID,
    ...
    if (tileCallbacks == tileCallbackMap.end()) {
        ...
        tileCallbackMap.insert(
            { tileID.canonical, std::vector<OverscaledIDFunctionTuple>(1, tuple) });
    } else {
        for (auto iter = tileCallbacks->second.begin(); iter != tileCallbacks->second.end();
             iter++) {
        ...
        tileCallbacks->second.emplace_back(
            std::make_tuple(tileID.overscaledZ, tileID.wrap, tileRef));
    }
    if (cachedTileData == dataCache.end()) {
...
void CustomTileLoader::invalidateRegion(const LatLngBounds& bounds, Range<uint8_t>) {
    ...
    for (auto idtuple = tileCallbackMap.begin(); idtuple != tileCallbackMap.end(); idtuple++) {
        ...
        if (tileRange == tileRanges.end()) {
            tileRange =
                tileRanges
                    .emplace(std::make_pair(zoom, util::TileRange::fromLatLngBounds(bounds, zoom)))
                    .first;
        }
