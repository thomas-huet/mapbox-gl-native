...
RasterDEMTile::RasterDEMTile(const OverscaledTileID& id_,
    ...
    : Tile(id_),
      ...
      worker(parameters.workerScheduler, ActorRef<RasterDEMTile>(*this, mailbox)) {
...
void RasterDEMTile::onParsed(std::unique_ptr<HillshadeBucket> result,
...
void RasterDEMTile::backfillBorder(const RasterDEMTile& borderTile, const DEMTileNeighbors mask) {
    ...
    if (borderBucket) {
        ...
        // update the bitmask to indicate that this tiles have been backfilled by flipping the
        // relevant bit
