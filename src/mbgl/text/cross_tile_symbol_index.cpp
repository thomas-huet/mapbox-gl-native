...
Point<int64_t> TileLayerIndex::getScaledCoordinates(SymbolInstance& symbolInstance,
                                                    const OverscaledTileID& childTileCoord) {
    // Round anchor positions to roughly 4 pixel grid
    const double roundingFactor = 512.0 / util::EXTENT / 2.0;
    const double scale =
        roundingFactor / std::pow(2, childTileCoord.canonical.z - coord.canonical.z);
    return {
        static_cast<int64_t>(std::floor(
            (childTileCoord.canonical.x * util::EXTENT + symbolInstance.anchor.point.x) * scale)),
        static_cast<int64_t>(std::floor(
            (childTileCoord.canonical.y * util::EXTENT + symbolInstance.anchor.point.y) * scale))
    };
}

void TileLayerIndex::findMatches(std::vector<SymbolInstance>& symbolInstances,
                                 const OverscaledTileID& newCoord,
                                 std::set<uint32_t>& zoomCrossTileIDs) {
    float tolerance = coord.canonical.z < newCoord.canonical.z
                          ? 1
                          : std::pow(2, coord.canonical.z - newCoord.canonical.z);
    ...
    if (wrapDelta != 0) {
        ...
        for (auto& zoomIndex : indexes) {
            ...
            for (auto& index : zoomIndex.second) {
                ...
                index.second.coord =
                    index.second.coord.unwrapTo(index.second.coord.wrap + wrapDelta);
...
bool CrossTileSymbolLayerIndex::addBucket(const OverscaledTileID& tileID,
    ...
    if (previousIndex != thisZoomIndexes.end()) {
        if (previousIndex->second.bucketInstanceId == bucket.bucketInstanceId) {
            ...
            // Remove the old bucket's "used crossTileIDs" now so that the new bucket can claim
            // them.
            ...
            for (auto& childIndex : zoomIndexes) {
                if (childIndex.second.coord.isChildOf(tileID)) {
                    childIndex.second.findMatches(bucket.symbolInstances, tileID,
                                                  usedCrossTileIDs[tileID.overscaledZ]);
                }
            }
        } else {
    ...
    indexes[tileID.overscaledZ].emplace(
        tileID, TileLayerIndex(tileID, bucket.symbolInstances, bucket.bucketInstanceId));
    return true;
}

void CrossTileSymbolLayerIndex::removeBucketCrossTileIDs(uint8_t zoom,
                                                         const TileLayerIndex& removedBucket) {
