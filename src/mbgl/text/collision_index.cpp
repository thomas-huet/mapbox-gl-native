...
CollisionIndex::CollisionIndex(const TransformState& transformState_)
    : transformState(transformState_),
      ...
      pitchFactor(std::cos(transformState.getPitch()) *
...
float CollisionIndex::approximateTileDistance(const TileDistance& tileDistance,
    ...
    // This is a quick and dirty solution for chosing which collision circles to use (since
    // collision circles are
    // laid out in tile units). Ideally, I think we should generate collision circles on the fly in
    // viewport coordinates
    // at the time we do collision detection.

    // incidenceStretch is the ratio of how much y space a label takes up on a tile while drawn
    // perpendicular to the viewport vs
    //  how much space it would take up if it were drawn flat on the tile
    // Using law of sines, camera_to_anchor/sin(ground_angle) =
    // camera_to_center/sin(incidence_angle)
...
bool CollisionIndex::isOffscreen(const CollisionBox& box) const {
    return box.px2 < viewportPadding || box.px1 >= screenRightBoundary ||
           box.py2 < viewportPadding || box.py1 >= screenBottomBoundary;
}

bool CollisionIndex::isInsideGrid(const CollisionBox& box) const {
    return box.px2 >= 0 && box.px1 < gridRightBoundary && box.py2 >= 0 &&
           box.py1 < gridBottomBoundary;
}
...
std::pair<bool, bool> CollisionIndex::placeFeature(CollisionFeature& feature,
    ...
    if (!feature.alongLine) {
        ...
        if (!isInsideGrid(box) ||
            (!allowOverlap &&
             collisionGrid.hitTest({ { box.px1, box.py1 }, { box.px2, box.py2 } }))) {
            return { false, false };
        }

        return { true, isOffscreen(box) };
    } else {
        return placeLineFeature(feature, posMatrix, labelPlaneMatrix, textPixelRatio, symbol, scale,
                                fontSize, allowOverlap, pitchWithMap, collisionDebug);
    }
}
...
std::pair<bool, bool> CollisionIndex::placeLineFeature(CollisionFeature& feature,
    ...
    const auto firstAndLastGlyph = placeFirstAndLastGlyph(
        fontScale, lineOffsetX, lineOffsetY,
        /*flip*/ false, labelPlaneAnchorPoint, tileUnitAnchorPoint, symbol, labelPlaneMatrix,
    ...
    if (firstAndLastGlyph) {
        firstTileDistance = approximateTileDistance(
            *(firstAndLastGlyph->first.tileDistance), firstAndLastGlyph->first.angle,
            pixelsToTileUnits, projectedAnchor.second, pitchWithMap);
        lastTileDistance = approximateTileDistance(
            *(firstAndLastGlyph->second.tileDistance), firstAndLastGlyph->second.angle,
            pixelsToTileUnits, projectedAnchor.second, pitchWithMap);
    }
            ...
            if (placedTooDensely) {
                ...
                if (atLeastOneMoreCircle) {
                    ...
                    if ((nextBoxDistanceFromAnchor > -firstTileDistance) &&
                        ...
                        // Hide significantly overlapping circles, unless this is the last one we
                        // can
                        // use, in which case we want to keep it in place even if it's tightly
                        // packed
        ...
        if (!allowOverlap) {
            if (collisionGrid.hitTest({ { circle.px, circle.py }, circle.radius })) {
                if (!collisionDebug) {
                    ...
                    // Don't early exit if we're showing the debug circles because we still want to
                    // calculate
...
void CollisionIndex::insertFeature(CollisionFeature& feature,
    ...
    if (feature.alongLine) {
        for (auto& circle : feature.boxes) {
            ...
            if (ignorePlacement) {
                ignoredGrid.insert(IndexedSubfeature(feature.indexedFeature, bucketInstanceId),
                                   { { circle.px, circle.py }, circle.radius });
            } else {
                collisionGrid.insert(IndexedSubfeature(feature.indexedFeature, bucketInstanceId),
                                     { { circle.px, circle.py }, circle.radius });
            }
        }
    } else {
...
bool polygonIntersectsBox(const LineString<float>& polygon,
                          const GridIndex<IndexedSubfeature>::BBox& bbox) {
...
std::unordered_map<uint32_t, std::vector<IndexedSubfeature>>
CollisionIndex::queryRenderedSymbols(const ScreenLineString& queryGeometry) const {
...
std::pair<float, float> CollisionIndex::projectAnchor(const mat4& posMatrix,
    ...
    return std::make_pair(0.5 + 0.5 * (transformState.getCameraToCenterDistance() / p[3]), p[3]);
}

std::pair<Point<float>, float>
CollisionIndex::projectAndGetPerspectiveRatio(const mat4& posMatrix,
                                              const Point<float>& point) const {
    vec4 p = { { point.x, point.y, 0, 1 } };
    matrix::transformMat4(p, p, posMatrix);
    return std::make_pair(
        Point<float>((((p[0] / p[3] + 1) / 2) * transformState.getSize().width) + viewportPadding,
                     (((-p[1] / p[3] + 1) / 2) * transformState.getSize().height) +
                         viewportPadding),
