...
SymbolQuad getIconQuad(const PositionedIcon& shapedIcon,
    ...
    // If you have a 10px icon that isn't perfectly aligned to the pixel grid it will cover 11
    // actual
    // pixels. The quad needs to be padded to account for this, otherwise they'll look slightly
    // clipped
    ...
    if (layout.get<IconTextFit>() != IconTextFitType::None && shapedText) {
        ...
        auto offsetY = layout.get<IconTextFit>() == IconTextFitType::Width
                           ? (textHeight - iconHeight) * 0.5
                           : 0;
        auto offsetX = layout.get<IconTextFit>() == IconTextFitType::Height
                           ? (textWidth - iconWidth) * 0.5
                           : 0;
        auto width = layout.get<IconTextFit>() == IconTextFitType::Width ||
                             layout.get<IconTextFit>() == IconTextFitType::Both
                         ? textWidth
                         : iconWidth;
        auto height = layout.get<IconTextFit>() == IconTextFitType::Height ||
                              layout.get<IconTextFit>() == IconTextFitType::Both
                          ? textHeight
                          : iconHeight;
    ...
    Rect<uint16_t> textureRect{ static_cast<uint16_t>(image.textureRect.x - border),
...
SymbolQuads getGlyphQuads(const Shaping& shapedText,
    ...
    for (const PositionedGlyph& positionedGlyph : shapedText.positionedGlyphs) {
        ...
        const bool alongLine = layout.get<TextRotationAlignment>() == AlignmentType::Map &&
                               placement == SymbolPlacementType::Line;

        const Point<float> glyphOffset =
            alongLine ? Point<float>{ positionedGlyph.x + halfAdvance, positionedGlyph.y }
                      : Point<float>{ 0.0f, 0.0f };

        const Point<float> builtInOffset = alongLine ? Point<float>{ 0.0f, 0.0f } : Point<float>{
