...
OpacityState::OpacityState(bool placed_, bool skipFade)
    : opacity((skipFade && placed_) ? 1 : 0), placed(placed_) {
}

OpacityState::OpacityState(const OpacityState& prevState, float increment, bool placed_)
    : opacity(
          ::fmax(0, ::fmin(1, prevState.opacity + (prevState.placed ? increment : -increment)))),
      placed(placed_) {
}
...
void Placement::placeLayer(RenderSymbolLayer& symbolLayer,
    ...
    for (RenderTile& renderTile : symbolLayer.renderTiles) {
        ...
        const float textPixelRatio =
            (util::tileSize * geometryTile.id.overscaleFactor()) / util::EXTENT;
        ...
        mat4 textLabelPlaneMatrix = getLabelPlaneMatrix(
            posMatrix, layout.get<style::TextPitchAlignment>() == style::AlignmentType::Map,
            layout.get<style::TextRotationAlignment>() == style::AlignmentType::Map, state,
            pixelsToTileUnits);

        mat4 iconLabelPlaneMatrix = getLabelPlaneMatrix(
            posMatrix, layout.get<style::IconPitchAlignment>() == style::AlignmentType::Map,
            layout.get<style::IconRotationAlignment>() == style::AlignmentType::Map, state,
            pixelsToTileUnits);

        // As long as this placement lives, we have to hold onto this bucket's
        // matching FeatureIndex/data for querying purposes
        retainedQueryData.emplace(
            std::piecewise_construct, std::forward_as_tuple(symbolBucket.bucketInstanceId),
            std::forward_as_tuple(symbolBucket.bucketInstanceId, geometryTile.getFeatureIndex(),
                                  geometryTile.id));

        placeLayerBucket(symbolBucket, posMatrix, textLabelPlaneMatrix, iconLabelPlaneMatrix, scale,
                         textPixelRatio, showCollisionBoxes, seenCrossTileIDs,
                         renderTile.tile.holdForFade());
    }
}

void Placement::placeLayerBucket(SymbolBucket& bucket,
    ...
    for (auto& symbolInstance : bucket.symbolInstances) {
        ...
        if (seenCrossTileIDs.count(symbolInstance.crossTileID) == 0) {
            if (holdingForFade) {
                ...
                // seenCrossTileIDs, because we don't
            ...
            if (symbolInstance.placedTextIndex) {
                PlacedSymbol& placedSymbol =
                    bucket.text.placedSymbols.at(*symbolInstance.placedTextIndex);
                const float fontSize =
                    evaluateSizeForFeature(partiallyEvaluatedTextSize, placedSymbol);

                auto placed = collisionIndex.placeFeature(
                    symbolInstance.textCollisionFeature, posMatrix, textLabelPlaneMatrix,
                    textPixelRatio, placedSymbol, scale, fontSize,
            ...
            if (symbolInstance.placedIconIndex) {
                PlacedSymbol& placedSymbol =
                    bucket.icon.placedSymbols.at(*symbolInstance.placedIconIndex);
                const float fontSize =
                    evaluateSizeForFeature(partiallyEvaluatedIconSize, placedSymbol);

                auto placed = collisionIndex.placeFeature(
                    symbolInstance.iconCollisionFeature, posMatrix, iconLabelPlaneMatrix,
                    textPixelRatio, placedSymbol, scale, fontSize,
            ...
            const bool iconWithoutText =
                !symbolInstance.hasText || bucket.layout.get<style::TextOptional>();
            const bool textWithoutIcon =
                !symbolInstance.hasIcon || bucket.layout.get<style::IconOptional>();
            ...
            if (placeText) {
                collisionIndex.insertFeature(symbolInstance.textCollisionFeature,
                                             bucket.layout.get<style::TextIgnorePlacement>(),
                                             bucket.bucketInstanceId);
            }

            if (placeIcon) {
                collisionIndex.insertFeature(symbolInstance.iconCollisionFeature,
                                             bucket.layout.get<style::IconIgnorePlacement>(),
                                             bucket.bucketInstanceId);
            }
            ...
            if (placements.find(symbolInstance.crossTileID) != placements.end()) {
                // If there's a previous placement with this ID, it comes from a tile that's fading
                // out
            ...
            placements.emplace(
                ...
                JointPlacement(placeText, placeIcon, offscreen || bucket.justReloaded));
...
void Placement::commit(const Placement& prevPlacement, TimePoint now) {
    ...
    // add the opacities from the current placement, and copy their current values from the previous
    // placement
    for (auto& jointPlacement : placements) {
        ...
        if (prevOpacity != prevPlacement.opacities.end()) {
            opacities.emplace(jointPlacement.first,
                              JointOpacityState(prevOpacity->second, increment,
                                                jointPlacement.second.text,
                                                jointPlacement.second.icon));
            placementChanged = placementChanged ||
                               jointPlacement.second.icon != prevOpacity->second.icon.placed ||
                               jointPlacement.second.text != prevOpacity->second.text.placed;
        } else {
            opacities.emplace(jointPlacement.first,
                              JointOpacityState(jointPlacement.second.text,
                                                jointPlacement.second.icon,
                                                jointPlacement.second.skipFade));
            placementChanged =
                placementChanged || jointPlacement.second.icon || jointPlacement.second.text;
        }
    }

    // copy and update values from the previous placement that aren't in the current placement but
    // haven't finished fading
    for (auto& prevOpacity : prevPlacement.opacities) {
        if (opacities.find(prevOpacity.first) == opacities.end()) {
            ...
            if (!jointOpacity.isHidden()) {
                ...
                placementChanged = placementChanged || prevOpacity.second.icon.placed ||
                                   prevOpacity.second.text.placed;
            }
        }
    }
...
void Placement::updateBucketOpacities(SymbolBucket& bucket, std::set<uint32_t>& seenCrossTileIDs) {
    ...
    if (bucket.hasCollisionBoxData())
        bucket.collisionBox.dynamicVertices.clear();
    if (bucket.hasCollisionCircleData())
        bucket.collisionCircle.dynamicVertices.clear();

    JointOpacityState duplicateOpacityState(false, false, true);

    JointOpacityState defaultOpacityState(bucket.layout.get<style::TextAllowOverlap>(),
                                          bucket.layout.get<style::IconAllowOverlap>(), true);
    ...
    for (SymbolInstance& symbolInstance : bucket.symbolInstances) {
        ...
        if (symbolInstance.hasText) {
            auto opacityVertex = SymbolOpacityAttributes::vertex(opacityState.text.placed,
            ...
            if (symbolInstance.placedTextIndex) {
                bucket.text.placedSymbols[*symbolInstance.placedTextIndex].hidden =
                    opacityState.isHidden();
            }
            if (symbolInstance.placedVerticalTextIndex) {
                bucket.text.placedSymbols[*symbolInstance.placedVerticalTextIndex].hidden =
                    opacityState.isHidden();
            }
        }
        if (symbolInstance.hasIcon) {
            auto opacityVertex = SymbolOpacityAttributes::vertex(opacityState.icon.placed,
            ...
            if (symbolInstance.placedIconIndex) {
                bucket.icon.placedSymbols[*symbolInstance.placedIconIndex].hidden =
                    opacityState.isHidden();
            }
        }
