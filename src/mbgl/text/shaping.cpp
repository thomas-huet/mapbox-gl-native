...
float calculateBadness(const float lineWidth,
...
std::set<std::size_t> determineLineBreaks(const std::u16string& logicalInput,
    ...
    for (std::size_t i = 0; i < logicalInput.size(); i++) {
        ...
        if (it != glyphs.end() && it->second &&
            !boost::algorithm::is_any_of(u" \t\n\v\f\r")(codePoint)) {
            currentX += (*it->second)->metrics.advance + spacing;
        }

        // Ideographic characters, spaces, and word-breaking punctuation that often appear without
        // surrounding spaces.
        if ((i < logicalInput.size() - 1) && (util::i18n::allowsWordBreaking(codePoint) ||
                                              util::i18n::allowsIdeographicBreaking(codePoint))) {
            potentialBreaks.push_back(
                evaluateBreak(i + 1, currentX, targetWidth, potentialBreaks,
                              calculatePenalty(codePoint, logicalInput[i + 1]), false));
        }
    }

    return leastBadBreaks(
        evaluateBreak(logicalInput.size(), currentX, targetWidth, potentialBreaks, 0, true));
}
...
void shapeLines(Shaping& shaping,
    ...
    const float justify = textJustify == style::TextJustifyType::Right
                              ? 1
                              : textJustify == style::TextJustifyType::Left ? 0 : 0.5;
    ...
    for (std::u16string line : lines) {
        ...
        for (char16_t chr : line) {
            ...
            if (writingMode == WritingModeType::Horizontal ||
                !util::i18n::hasUprightVerticalOrientation(chr)) {
...
const Shaping getShaping(const std::u16string& logicalInput,
    ...
    std::vector<std::u16string> reorderedLines = bidi.processText(
        logicalInput, determineLineBreaks(logicalInput, spacing, maxWidth, writingMode, glyphs));

    shapeLines(shaping, reorderedLines, spacing, lineHeight, textAnchor, textJustify,
               verticalHeight, writingMode, glyphs);
