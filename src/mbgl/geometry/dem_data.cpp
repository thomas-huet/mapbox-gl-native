...
DEMData::DEMData(const PremultipliedImage& _image, Tileset::DEMEncoding encoding)
    ...
    // in order to avoid flashing seams between tiles, here we are initially populating a 1px border
    // of
    // pixels around the image with the data of the nearest pixel from the image. this data is
    // eventually
    // replaced when the tile's neighboring tiles are loaded and the accurate data can be backfilled
    // using
