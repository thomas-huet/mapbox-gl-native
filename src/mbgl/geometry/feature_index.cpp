...
void FeatureIndex::query(std::unordered_map<std::string, std::vector<Feature>>& result,
    ...
    const int16_t additionalPadding =
        std::min<int16_t>(util::EXTENT, additionalQueryPadding * pixelsToTileUnits);

    // Query the grid index
    mapbox::geometry::box<int16_t> box = mapbox::geometry::envelope(queryGeometry);
    std::vector<IndexedSubfeature> features =
        grid.query({ convertPoint<float>(box.min - additionalPadding),
                     convertPoint<float>(box.max + additionalPadding) });

    std::sort(features.begin(), features.end(),
              [](const IndexedSubfeature& a, const IndexedSubfeature& b) {
    ...
    for (const auto& indexedFeature : features) {
        ...
        addFeature(result, indexedFeature, queryOptions, tileID.canonical, layers, queryGeometry,
                   transformState, pixelsToTileUnits, posMatrix);
    }
}
...
std::unordered_map<std::string, std::vector<Feature>> FeatureIndex::lookupSymbolFeatures(
    ...
    std::sort(sortedFeatures.begin(), sortedFeatures.end(),
              [featureSortOrder](const IndexedSubfeature& a, const IndexedSubfeature& b) {
                  // Same idea as the non-symbol sort order, but symbol features may have changed
                  // their sort order
                  // since their corresponding IndexedSubfeature was added to the CollisionIndex
                  // The 'featureSortOrder' is relatively inefficient for querying but cheap to
                  // build on every bucket sort
                  if (featureSortOrder) {
                      ...
                      auto sortedA =
                          std::find(featureSortOrder->begin(), featureSortOrder->end(), a.index);
                      auto sortedB =
                          std::find(featureSortOrder->begin(), featureSortOrder->end(), b.index);
                      assert(sortedA != featureSortOrder->end());
                      assert(sortedB != featureSortOrder->end());
                      return sortedA > sortedB;
                  } else {
                      // Bucket hasn't been re-sorted based on angle, so use same "reverse of
                      // appearance in source data"
    ...
    for (const auto& symbolFeature : sortedFeatures) {
        mat4 unusedMatrix;
        addFeature(result, symbolFeature, queryOptions, tileID.canonical, layers,
                   GeometryCoordinates(), {}, 0, unusedMatrix);
    }
    return result;
}
...
void FeatureIndex::addFeature(std::unordered_map<std::string, std::vector<Feature>>& result,
    ...
    for (const std::string& layerID : bucketLayerIDs.at(indexedFeature.bucketLeaderID)) {
        ...
        if (!renderLayer->is<RenderSymbolLayer>() &&
            !renderLayer->queryIntersectsFeature(queryGeometry, *geometryTileFeature, tileID.z,
                                                 transformState, pixelsToTileUnits, posMatrix)) {
            continue;
        }

        if (options.filter &&
            !(*options.filter)(style::expression::EvaluationContext{ static_cast<float>(tileID.z),
...
optional<GeometryCoordinates>
FeatureIndex::translateQueryGeometry(const GeometryCoordinates& queryGeometry,
...
void FeatureIndex::setBucketLayerIDs(const std::string& bucketLeaderID,
