...
struct SymbolLayoutAttributes
    : gl::Attributes<attributes::a_pos_offset, attributes::a_data<uint16_t, 4>> {
    static Vertex vertex(Point<float> labelAnchor,
        ...
        return Vertex{ // combining pos and offset to reduce number of vertex attributes passed to
                       // shader (8 max for some devices)
                       { { static_cast<int16_t>(labelAnchor.x), static_cast<int16_t>(labelAnchor.y),
                           static_cast<int16_t>(::round(o.x * 32)), // use 1/32 pixels for placement
                           static_cast<int16_t>(::round((o.y + glyphOffsetY) * 32)) } },
                       { { tx, ty, static_cast<uint16_t>(sizeData.min * 10),
...
struct SymbolDynamicLayoutAttributes : gl::Attributes<attributes::a_projected_pos> {
    static Vertex vertex(Point<float> anchorPoint, float labelAngle) {
        return Vertex{ { { anchorPoint.x, anchorPoint.y, labelAngle } } };
    }
};

struct SymbolOpacityAttributes : gl::Attributes<attributes::a_fade_opacity> {
    static Vertex vertex(bool placed, float opacity) {
        return Vertex{ { { static_cast<uint8_t>((static_cast<uint8_t>(opacity * 127) << 1) |
                                                static_cast<uint8_t>(placed)) } } };
    }
};
...
// particular attribute & uniform logic needed by each possible type of the {Text,Icon}Size
// properties.
class SymbolSizeBinder {
public:
    virtual ~SymbolSizeBinder() = default;

    using Uniforms = gl::Uniforms<uniforms::u_is_size_zoom_constant,
                                  uniforms::u_is_size_feature_constant,
                                  uniforms::u_size_t,
                                  uniforms::u_size>;
    using UniformValues = Uniforms::Values;

    static std::unique_ptr<SymbolSizeBinder>
    create(const float tileZoom,
    ...
    UniformValues uniformValues(float currentZoom) const {
        ...
        return UniformValues{ uniforms::u_is_size_zoom_constant::Value{ u.isZoomConstant },
...
public:
    ...
    ConstantSymbolSizeBinder(const float tileZoom,
                             const style::CameraFunction<float>& function_,
                             const float /*defaultValue*/)
        : layoutSize(function_.evaluate(tileZoom + 1)), function(function_) {
        const Range<float> zoomLevels = function_.getCoveringStops(tileZoom, tileZoom + 1);
        coveringRanges =
            std::make_tuple(zoomLevels, Range<float>{ function_.evaluate(zoomLevels.min),
                                                      function_.evaluate(zoomLevels.max) });
    }
    ...
    ZoomEvaluatedSize evaluateForZoom(float currentZoom) const override {
        ...
        if (coveringRanges) {
            ...
            float t =
                util::clamp(function->interpolationFactor(zoomLevels, currentZoom), 0.0f, 1.0f);
...
public:
    ...
    SourceFunctionSymbolSizeBinder(const float /*tileZoom*/,
                                   const style::SourceFunction<float>& function_,
                                   const float defaultValue_)
        : function(function_), defaultValue(defaultValue_) {
    }
...
public:
    CompositeFunctionSymbolSizeBinder(const float tileZoom,
                                      const style::CompositeFunction<float>& function_,
                                      const float defaultValue_)
        : function(function_),
    ...
    Range<float> getVertexSizeData(const GeometryTileFeature& feature) override {
        return { function.evaluate(coveringZoomStops.min, feature, defaultValue),
                 function.evaluate(coveringZoomStops.max, feature, defaultValue) };
    };

    ZoomEvaluatedSize evaluateForZoom(float currentZoom) const override {
        float sizeInterpolationT =
            util::clamp(function.interpolationFactor(coveringZoomStops, currentZoom), 0.0f, 1.0f);
...
public:
    ...
    using LayoutAndSizeAttributes = gl::ConcatenateAttributes<
        LayoutAttributes,
        gl::ConcatenateAttributes<SymbolDynamicLayoutAttributes, SymbolOpacityAttributes>>;
    ...
    using AllUniforms =
        gl::ConcatenateUniforms<Uniforms, gl::ConcatenateUniforms<SizeUniforms, PaintUniforms>>;
...
class SymbolIconProgram : public SymbolProgram<shaders::symbol_icon,
                                               gl::Triangle,
...
enum class SymbolSDFPart { Fill = 1, Halo = 0 };

template <class PaintProperties>
class SymbolSDFProgram : public SymbolProgram<shaders::symbol_sdf,
                                              gl::Triangle,
                                              SymbolLayoutAttributes,
                                              gl::Uniforms<uniforms::u_matrix,
                                                           uniforms::u_label_plane_matrix,
