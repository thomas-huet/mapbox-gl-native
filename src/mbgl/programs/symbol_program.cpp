...
std::unique_ptr<SymbolSizeBinder>
SymbolSizeBinder::create(const float tileZoom,
    ...
    return sizeProperty.match(
        [&](const style::CompositeFunction<float>& function) -> std::unique_ptr<SymbolSizeBinder> {
            return std::make_unique<CompositeFunctionSymbolSizeBinder>(tileZoom, function,
                                                                       defaultValue);
        },
        [&](const style::SourceFunction<float>& function) {
            return std::make_unique<SourceFunctionSymbolSizeBinder>(tileZoom, function,
                                                                    defaultValue);
        },
...
Values makeValues(const bool isText,
    ...
    if (values.pitchAlignment == AlignmentType::Map) {
        ...
        extrudeScale = { { pixelsToGLUnits[0] * state.getCameraToCenterDistance(),
    ...
    if (alongLine) {
        ...
        labelPlaneMatrix =
            getLabelPlaneMatrix(tile.matrix, pitchWithMap, rotateWithMap, state, pixelsToTileUnits);
    }

    mat4 glCoordMatrix =
        getGlCoordMatrix(tile.matrix, pitchWithMap, rotateWithMap, state, pixelsToTileUnits);

    return Values{
        uniforms::u_matrix::Value{
            tile.translatedMatrix(values.translate, values.translateAnchor, state) },
        uniforms::u_label_plane_matrix::Value{ labelPlaneMatrix },
        uniforms::u_gl_coord_matrix::Value{ tile.translateVtxMatrix(
            glCoordMatrix, values.translate, values.translateAnchor, state, true) },
        uniforms::u_extrude_scale::Value{ extrudeScale }, uniforms::u_texsize::Value{ texsize },
        uniforms::u_texture::Value{ 0 }, uniforms::u_fade_change::Value{ symbolFadeChange },
...
SymbolIconProgram::uniformValues(const bool isText,
                                 ...
                                 const TransformState& state,
                                 const float symbolFadeChange) {
    return makeValues<SymbolIconProgram::UniformValues>(isText, values, texsize, pixelsToGLUnits,
                                                        alongLine, tile, state, symbolFadeChange);
}

template <class PaintProperties>
typename SymbolSDFProgram<PaintProperties>::UniformValues
SymbolSDFProgram<PaintProperties>::uniformValues(const bool isText,
                                                 const style::SymbolPropertyValues& values,
                                                 const Size& texsize,
                                                 const std::array<float, 2>& pixelsToGLUnits,
                                                 const bool alongLine,
    ...
    return makeValues<SymbolSDFProgram<PaintProperties>::UniformValues>(
        isText, values, texsize, pixelsToGLUnits, alongLine, tile, state, symbolFadeChange,
