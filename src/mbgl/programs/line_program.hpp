...
struct LineLayoutAttributes
    : gl::Attributes<attributes::a_pos_normal, attributes::a_data<uint8_t, 4>> {};
...
public:
    ...
    static LayoutVertex layoutVertex(
        ...
        return LayoutVertex{
            { { p.x, p.y, static_cast<int16_t>(round ? 1 : 0),
            ...
            { { // add 128 to store a byte in an unsigned byte
                ...
                // Encode the -1/0/1 direction value into .zw coordinates of a_data, which is
                // normally covered
                // by linesofar, so we need to merge them.
                // The z component's first bit, as well as the sign bit is reserved for the
                // direction,
                // so we need to shift the linesofar.
                static_cast<uint8_t>(((dir == 0 ? 0 : (dir < 0 ? -1 : 1)) + 1) |
                                     ((linesofar & 0x3F) << 2)),
...
class LinePatternProgram : public Program<shaders::line_pattern,
                                          gl::Triangle,
                                          LineLayoutAttributes,
...
class LineSDFProgram : public Program<shaders::line_sdf,
                                      gl::Triangle,
                                      LineLayoutAttributes,
                                      gl::Uniforms<uniforms::u_matrix,
                                                   uniforms::u_ratio,
