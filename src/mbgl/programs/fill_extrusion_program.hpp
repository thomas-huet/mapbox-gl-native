...
struct FillExtrusionLayoutAttributes : gl::Attributes<attributes::a_pos, attributes::a_normal_ed> {
};

struct FillExtrusionUniforms : gl::Uniforms<uniforms::u_matrix,
                                            uniforms::u_lightcolor,
                                            uniforms::u_lightpos,
                                            uniforms::u_lightintensity> {
    static Values values(mat4, const TransformState&, const EvaluatedLight&);
};

struct FillExtrusionPatternUniforms : gl::Uniforms<uniforms::u_matrix,
...
class FillExtrusionProgram : public Program<shaders::fill_extrusion,
                                            gl::Triangle,
...
public:
    ...
    static LayoutVertex
    layoutVertex(Point<int16_t> p, double nx, double ny, double nz, unsigned short t, uint16_t e) {
        const auto factor = pow(2, 13);

        return LayoutVertex{
            { { p.x, p.y } },
            { { // Multiply normal vector components by 2^14 to pack them into integers
                // We pack a bool (`t`) into the x component indicating whether it is an upper or
                // lower vertex
                static_cast<int16_t>(floor(nx * factor) * 2 + t),
                static_cast<int16_t>(ny * factor * 2), static_cast<int16_t>(nz * factor * 2),
...
class FillExtrusionPatternProgram : public Program<shaders::fill_extrusion_pattern,
                                                   gl::Triangle,
                                                   FillExtrusionLayoutAttributes,
