...
mat4 RenderTile::translateVtxMatrix(const mat4& tileMatrix,
    ...
    if (inViewportPixelUnits) {
        ...
        matrix::translate(vtxMatrix, tileMatrix, id.pixelsToTileUnits(translate.x, state.getZoom()),
                          id.pixelsToTileUnits(translate.y, state.getZoom()), 0);
    }
...
void RenderTile::finishRender(PaintParameters& parameters) {
    ...
    if (parameters.debugOptions & (MapDebugOptions::Timestamps | MapDebugOptions::ParseStatus)) {
        ...
        const auto allAttributeBindings = program.computeAllAttributeBindings(
            *tile.debugBucket->vertexBuffer, paintAttributeData, properties);

        program.draw(parameters.context, gl::Lines{ 4.0f * parameters.pixelRatio },
                     gl::DepthMode::disabled(), parameters.stencilModeForClipping(clip),
                     gl::ColorMode::unblended(), *tile.debugBucket->indexBuffer,
                     ...
                     allAttributeBindings, "debug");

        program.draw(parameters.context, gl::Lines{ 2.0f * parameters.pixelRatio },
                     gl::DepthMode::disabled(), parameters.stencilModeForClipping(clip),
                     gl::ColorMode::unblended(), *tile.debugBucket->indexBuffer,
                     tile.debugBucket->segments,
                     program.computeAllUniformValues(
    ...
    if (parameters.debugOptions & MapDebugOptions::TileBorders) {
        parameters.programs.debug.draw(
            ...
            gl::DepthMode::disabled(), parameters.stencilModeForClipping(clip),
            gl::ColorMode::unblended(), parameters.staticData.tileBorderIndexBuffer,
