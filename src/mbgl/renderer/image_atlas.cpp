...
ImageAtlas makeImageAtlas(const ImageMap& images) {
    ...
    for (const auto& entry : images) {
        ...
        const mapbox::Bin& bin = *pack.packOne(-1, image.image.size.width + 2 * padding,
                                               image.image.size.height + 2 * padding);

        result.image.resize(
            { static_cast<uint32_t>(pack.width()), static_cast<uint32_t>(pack.height()) });

        PremultipliedImage::copy(image.image, result.image, { 0, 0 },
                                 { bin.x + padding, bin.y + padding }, image.image.size);

        result.positions.emplace(image.id, ImagePosition{ bin, image });
    }
