...
RenderFillLayer::RenderFillLayer(Immutable<style::FillLayer::Impl> _impl)
    : RenderLayer(style::LayerType::Fill, _impl), unevaluated(impl().paint.untransitioned()) {
}
...
RenderFillLayer::createBucket(const BucketParameters& parameters,
...
void RenderFillLayer::render(PaintParameters& parameters, RenderSource*) {
    if (evaluated.get<FillPattern>().from.empty()) {
        for (const RenderTile& tile : renderTiles) {
            ...
            auto draw = [&](auto& program, const auto& drawMode, const auto& depthMode,
                            const auto& indexBuffer, const auto& segments) {
                ...
                programInstance.draw(parameters.context, drawMode, depthMode,
                                     parameters.stencilModeForClipping(tile.clip),
                                     parameters.colorModeForRenderPass(), indexBuffer, segments,
                                     allUniformValues, allAttributeBindings, getID());
            };
        ...
        optional<ImagePosition> imagePosA =
            parameters.imageManager.getPattern(evaluated.get<FillPattern>().from);
        optional<ImagePosition> imagePosB =
            parameters.imageManager.getPattern(evaluated.get<FillPattern>().to);
        ...
        for (const RenderTile& tile : renderTiles) {
            ...
            auto draw = [&](auto& program, const auto& drawMode, const auto& depthMode,
                ...
                const auto allUniformValues = programInstance.computeAllUniformValues(
                    FillPatternUniforms::values(
                        ...
                        evaluated.get<FillPattern>(), tile.id, parameters.state),
                    paintPropertyBinders, evaluated, parameters.state.getZoom());
                const auto allAttributeBindings = programInstance.computeAllAttributeBindings(
                    *bucket.vertexBuffer, paintPropertyBinders, evaluated);
            ...
            draw(parameters.programs.fillPattern, gl::Triangles(),
                 parameters.depthModeForSublayer(1, gl::DepthMode::ReadWrite),
                 *bucket.triangleIndexBuffer, bucket.triangleSegments);

            if (evaluated.get<FillAntialias>() &&
                unevaluated.get<FillOutlineColor>().isUndefined()) {
                draw(parameters.programs.fillOutlinePattern, gl::Lines{ 2.0f },
...
bool RenderFillLayer::queryIntersectsFeature(const GeometryCoordinates& queryGeometry,
    ...
    auto translatedQueryGeometry = FeatureIndex::translateQueryGeometry(
        queryGeometry, evaluated.get<style::FillTranslate>(),
        evaluated.get<style::FillTranslateAnchor>(), transformState.getAngle(), pixelsToTileUnits);
