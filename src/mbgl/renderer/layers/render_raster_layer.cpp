...
RenderRasterLayer::RenderRasterLayer(Immutable<style::RasterLayer::Impl> _impl)
    : RenderLayer(style::LayerType::Raster, _impl), unevaluated(impl().paint.untransitioned()) {
}
...
RenderRasterLayer::createBucket(const BucketParameters&,
...
void RenderRasterLayer::render(PaintParameters& parameters, RenderSource* source) {
    ...
    auto draw = [&](const mat4& matrix, const auto& vertexBuffer, const auto& indexBuffer,
        ...
        const auto allUniformValues = programInstance.computeAllUniformValues(
            RasterProgram::UniformValues{
                uniforms::u_matrix::Value{ matrix }, uniforms::u_image0::Value{ 0 },
                ...
                uniforms::u_saturation_factor::Value{
                    saturationFactor(evaluated.get<RasterSaturation>()) },
                uniforms::u_contrast_factor::Value{
                    contrastFactor(evaluated.get<RasterContrast>()) },
                uniforms::u_spin_weights::Value{ spinWeights(evaluated.get<RasterHueRotate>()) },
                uniforms::u_buffer_scale::Value{ 1.0f }, uniforms::u_scale_parent::Value{ 1.0f },
    ...
    if (RenderImageSource* imageSource = source->as<RenderImageSource>()) {
        if (imageSource->isEnabled() && imageSource->isLoaded() &&
            !imageSource->bucket->needsUpload()) {
            ...
            for (auto matrix_ : imageSource->matrices) {
                draw(matrix_, *bucket.vertexBuffer, *bucket.indexBuffer, bucket.segments);
            }
        }
    } else {
