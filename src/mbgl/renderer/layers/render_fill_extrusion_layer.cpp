...
RenderFillExtrusionLayer::createBucket(const BucketParameters& parameters,
    ...
    if (parameters.pass == RenderPass::Pass3D) {
        ...
        auto draw = [&](auto& programInstance, const auto& tileBucket, auto&& uniformValues) {
            ...
            const auto allAttributeBindings = programInstance.computeAllAttributeBindings(
                *tileBucket.vertexBuffer, paintPropertyBinders, evaluated);
            ...
            programInstance.draw(parameters.context, gl::Triangles(),
                                 ...
                                 *tileBucket.indexBuffer, tileBucket.triangleSegments,
        ...
        if (evaluated.get<FillExtrusionPattern>().from.empty()) {
            for (const RenderTile& tile : renderTiles) {
                ...
                draw(parameters.programs.fillExtrusion.get(evaluated), bucket,
            ...
            for (const RenderTile& tile : renderTiles) {
                ...
                draw(parameters.programs.fillExtrusionPattern.get(evaluated), bucket,
...
bool RenderFillExtrusionLayer::queryIntersectsFeature(const GeometryCoordinates& queryGeometry,
    ...
    auto translatedQueryGeometry = FeatureIndex::translateQueryGeometry(
        queryGeometry, evaluated.get<style::FillExtrusionTranslate>(),
        evaluated.get<style::FillExtrusionTranslateAnchor>(), transformState.getAngle(),
        pixelsToTileUnits);
