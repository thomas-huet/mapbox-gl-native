...
RenderCircleLayer::RenderCircleLayer(Immutable<style::CircleLayer::Impl> _impl)
    : RenderLayer(style::LayerType::Circle, _impl), unevaluated(impl().paint.untransitioned()) {
}
...
RenderCircleLayer::createBucket(const BucketParameters& parameters,
    ...
    for (const RenderTile& tile : renderTiles) {
        ...
        const auto allUniformValues = programInstance.computeAllUniformValues(
            CircleProgram::UniformValues{
                uniforms::u_matrix::Value{ tile.translatedMatrix(
                    evaluated.get<CircleTranslate>(), evaluated.get<CircleTranslateAnchor>(),
                    parameters.state) },
                uniforms::u_scale_with_map::Value{ scaleWithMap },
                uniforms::u_extrude_scale::Value{
                    pitchWithMap ? std::array<float, 2>{ { tile.id.pixelsToTileUnits(
                                                               1, parameters.state.getZoom()),
            ...
            paintPropertyBinders, evaluated, parameters.state.getZoom());
...
GeometryCoordinate
projectPoint(const GeometryCoordinate& p, const mat4& posMatrix, const Size& size) {
...
bool RenderCircleLayer::queryIntersectsFeature(const GeometryCoordinates& queryGeometry,
    ...
    const GeometryCoordinates& translatedQueryGeometry =
        FeatureIndex::translateQueryGeometry(queryGeometry, evaluated.get<style::CircleTranslate>(),
                                             evaluated.get<style::CircleTranslateAnchor>(),
                                             transformState.getAngle(), pixelsToTileUnits)
    ...
    // A circle with fixed scaling relative to the viewport gets larger in tile space as it moves
    // into the distance
    // A circle with fixed scaling relative to the map gets smaller in viewport space as it moves
    // into the distance
    bool alignWithMap =
        evaluated.evaluate<style::CirclePitchAlignment>(zoom, feature) == AlignmentType::Map;
    ...
    for (auto& ring : geometry) {
        for (auto& point : ring) {
            ...
            if (pitchScale == CirclePitchScaleType::Viewport &&
                pitchAlignment == AlignmentType::Map) {
                adjustedSize *= center[3] / transformState.getCameraToCenterDistance();
            } else if (pitchScale == CirclePitchScaleType::Map &&
                       pitchAlignment == AlignmentType::Viewport) {
                adjustedSize *= transformState.getCameraToCenterDistance() / center[3];
            }

            if (util::polygonIntersectsBufferedPoint(transformedQueryGeometry, transformedPoint,
                                                     adjustedSize))
                return true;
        }
    }
