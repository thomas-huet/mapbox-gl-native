...
RenderBackgroundLayer::RenderBackgroundLayer(Immutable<style::BackgroundLayer::Impl> _impl)
    : RenderLayer(style::LayerType::Background, _impl), unevaluated(impl().paint.untransitioned()) {
}
...
RenderBackgroundLayer::createBucket(const BucketParameters&,
...
void RenderBackgroundLayer::evaluate(const PropertyEvaluationParameters& parameters) {
    ...
    passes =
        evaluated.get<style::BackgroundOpacity>() > 0 ? RenderPass::Translucent : RenderPass::None;
}
...
void RenderBackgroundLayer::render(PaintParameters& parameters, RenderSource*) {
    ...
    auto draw = [&](auto& program, auto&& uniformValues) {
        const auto allUniformValues = program.computeAllUniformValues(
            std::move(uniformValues), paintAttributeData, properties, parameters.state.getZoom());
        ...
        program.draw(parameters.context, gl::Triangles(),
                     parameters.depthModeForSublayer(0, gl::DepthMode::ReadOnly),
                     gl::StencilMode::disabled(), parameters.colorModeForRenderPass(),
                     parameters.staticData.quadTriangleIndexBuffer,
                     parameters.staticData.tileTriangleSegments, allUniformValues,
    ...
    if (!evaluated.get<BackgroundPattern>().to.empty()) {
        ...
        for (const auto& tileID :
             util::tileCover(parameters.state, parameters.state.getIntegerZoom())) {
            draw(parameters.programs.backgroundPattern,
                 BackgroundPatternUniforms::values(
                     parameters.matrixForTile(tileID), evaluated.get<BackgroundOpacity>(),
                     parameters.imageManager.getPixelSize(), *imagePosA, *imagePosB,
                     evaluated.get<BackgroundPattern>(), tileID, parameters.state));
        }
    } else {
        for (const auto& tileID :
             util::tileCover(parameters.state, parameters.state.getIntegerZoom())) {
            draw(parameters.programs.background,
