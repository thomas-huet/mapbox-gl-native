...
RenderLineLayer::RenderLineLayer(Immutable<style::LineLayer::Impl> _impl)
    : RenderLayer(style::LayerType::Line, _impl), unevaluated(impl().paint.untransitioned()) {
}
...
RenderLineLayer::createBucket(const BucketParameters& parameters,
    ...
    for (const RenderTile& tile : renderTiles) {
        ...
        auto draw = [&](auto& program, auto&& uniformValues) {
            ...
            const auto allUniformValues = programInstance.computeAllUniformValues(
                std::move(uniformValues), paintPropertyBinders, evaluated,
            ...
            programInstance.draw(parameters.context, gl::Triangles(),
                                 ...
                                 parameters.colorModeForRenderPass(), *bucket.indexBuffer,
                                 bucket.segments, allUniformValues, allAttributeBindings, getID());
        };
        ...
        if (!evaluated.get<LineDasharray>().from.empty()) {
            ...
            LinePatternPos posA =
                parameters.lineAtlas.getDashPosition(evaluated.get<LineDasharray>().from, cap);
            LinePatternPos posB =
                parameters.lineAtlas.getDashPosition(evaluated.get<LineDasharray>().to, cap);

            parameters.lineAtlas.bind(parameters.context, 0);

            draw(parameters.programs.lineSDF,
                 LineSDFProgram::uniformValues(evaluated, parameters.pixelRatio, tile,
                                               parameters.state, parameters.pixelsToGLUnits, posA,
                                               posB, parameters.lineAtlas.getSize().width));

        } else if (!evaluated.get<LinePattern>().from.empty()) {
            optional<ImagePosition> posA =
                parameters.imageManager.getPattern(evaluated.get<LinePattern>().from);
            optional<ImagePosition> posB =
                parameters.imageManager.getPattern(evaluated.get<LinePattern>().to);
            ...
            draw(parameters.programs.linePattern,
                 LinePatternProgram::uniformValues(
                     ...
                     parameters.imageManager.getPixelSize(), *posA, *posB));

        } else {
            draw(parameters.programs.line,
                 LineProgram::uniformValues(evaluated, tile, parameters.state,
...
bool RenderLineLayer::queryIntersectsFeature(const GeometryCoordinates& queryGeometry,
    ...
    auto translatedQueryGeometry = FeatureIndex::translateQueryGeometry(
        queryGeometry, evaluated.get<style::LineTranslate>(),
        evaluated.get<style::LineTranslateAnchor>(), transformState.getAngle(), pixelsToTileUnits);

    // Evaluate function
    auto offset = evaluated.get<style::LineOffset>().evaluate(feature, zoom,
                                                              style::LineOffset::defaultValue()) *
                  pixelsToTileUnits;
    ...
    return util::polygonIntersectsBufferedMultiLine(
        ...
        offsetGeometry.value_or(feature.getGeometries()), halfWidth);
}

float RenderLineLayer::getLineWidth(const GeometryTileFeature& feature, const float zoom) const {
    float lineWidth =
        evaluated.get<style::LineWidth>().evaluate(feature, zoom, style::LineWidth::defaultValue());
    float gapWidth = evaluated.get<style::LineGapWidth>().evaluate(
        feature, zoom, style::LineGapWidth::defaultValue());
    if (gapWidth) {
