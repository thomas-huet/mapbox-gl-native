...
#ifndef NDEBUG
    const ExtensionFunction<void(GLenum source, GLuint id, GLsizei length, const GLchar* message)>
        pushDebugGroup;

    const ExtensionFunction<void()> popDebugGroup;

    const ExtensionFunction<void(GLsizei length, const GLchar* marker)> pushGroupMarkerEXT;
