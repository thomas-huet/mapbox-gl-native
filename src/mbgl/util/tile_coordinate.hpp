...
public:
    ...
    static GeometryCoordinate toGeometryCoordinate(const UnwrappedTileID& tileID,
                                                   const TileCoordinatePoint& point) {
        ...
        return { int16_t(util::clamp<int64_t>(
                     (zoomed.p.x - tileID.canonical.x - tileID.wrap * scale) * util::EXTENT,
