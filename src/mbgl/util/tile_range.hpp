...
public:
    ...
    static TileRange
    fromLatLngBounds(const LatLngBounds& bounds, uint8_t minZoom, uint8_t maxZoom) {
        if (minZoom > maxZoom) {
        ...
        auto neProj =
            Projection::project(ne.longitude() > util::LONGITUDE_MAX ? ne.wrapped() : ne, maxZoom);
    ...
    bool contains(const CanonicalTileID& tileID) {
        if (tileID.z <= zoomRange.max && tileID.z >= zoomRange.min) {
            ...
            return (range.min.x > range.max.x ? // For wrapped bounds
                   ...
                   tileID.y <= y1 && tileID.y >= y0;
        }
        return false;
    }

private:
    TileRange(Range<Point<uint32_t>> range_, Range<uint8_t> z_) : range(range_), zoomRange(z_) {
    }
};
