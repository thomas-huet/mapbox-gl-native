...
optional<Color> Color::parse(const std::string& s) {
    ...
    if (css_color) {
        ...
        return { { css_color->r * factor, css_color->g * factor, css_color->b * factor,
                   css_color->a } };
    } else {
        return {};
    }
...
std::string Color::stringify() const {
    ...
    return "rgba(" + util::toString(array[0]) + "," + util::toString(array[1]) + "," +
           util::toString(array[2]) + "," + util::toString(array[3]) + ")";
}
