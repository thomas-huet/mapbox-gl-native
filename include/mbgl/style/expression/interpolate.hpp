...
public:
    ...
    double interpolationFactor(const Range<double>& inputLevels, const double input) const {
        return util::interpolationFactor(base, Range<float>{ static_cast<float>(inputLevels.min),
...
public:
    Interpolate(type::Type type_,
                ...
                std::map<double, std::unique_ptr<Expression>> stops_)
        : InterpolateBase(
              std::move(type_), std::move(interpolator_), std::move(input_), std::move(stops_)) {
        static_assert(util::Interpolatable<T>::value,
                      "Interpolate expression requires an interpolatable value type.");
    }
    ...
    EvaluationResult evaluate(const EvaluationContext& params) const override {
        ...
        if (it == stops.end()) {
            ...
            if (!lower->is<T>()) {
                return EvaluationError{ "Expected value to be of type " +
                                        toString(valueTypeToExpressionType<T>()) + ", but found " +
                                        toString(typeOf(*lower)) + " instead." };
            }

            if (!upper->is<T>()) {
                return EvaluationError{ "Expected value to be of type " +
                                        toString(valueTypeToExpressionType<T>()) + ", but found " +
                                        toString(typeOf(*upper)) + " instead." };
            }
    ...
    bool operator==(const Expression& e) const override {
        if (auto rhs = dynamic_cast<const Interpolate*>(&e)) {
            if (interpolator != rhs->interpolator || *input != *(rhs->input) ||
