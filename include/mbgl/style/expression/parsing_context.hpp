...
class Scope {
public:
    Scope(const std::map<std::string, std::shared_ptr<Expression>>& bindings_,
...
public:
    ...
    ParsingContext(std::string key_)
        : key(std::move(key_)), errors(std::make_shared<std::vector<ParsingError>>()) {
    }
    ...
    optional<type::Type> getExpected() const {
        return expected;
    }
    ...
    enum TypeAnnotationOption { includeTypeAnnotations, omitTypeAnnotations };
