...
private:
    ...
    friend bool operator==(const HeatmapColorPropertyValue& lhs,
                           const HeatmapColorPropertyValue& rhs) {
...
public:
    ...
    HeatmapColorPropertyValue(std::shared_ptr<expression::Expression> value_)
    ...
    Color evaluate(double heatmapDensity) const {
        const auto result =
            value->evaluate(expression::EvaluationContext({}, nullptr, { heatmapDensity }));
    ...
    bool hasDataDrivenPropertyDifference(const HeatmapColorPropertyValue&) const {
        return false;
    }
