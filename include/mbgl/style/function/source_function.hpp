...
public:
    using Stops = std::conditional_t<
        ...
        variant<ExponentialStops<T>, IntervalStops<T>, CategoricalStops<T>, IdentityStops<T>>,
        variant<IntervalStops<T>, CategoricalStops<T>, IdentityStops<T>>>;

    SourceFunction(std::unique_ptr<expression::Expression> expression_)
        : isExpression(true), expression(std::move(expression_)) {
        ...
        : isExpression(false),
          ...
          expression(stops.match(
              [&](const IdentityStops<T>&) {
                  return expression::Convert::fromIdentityFunction(
                      expression::valueTypeToExpressionType<T>(), property);
              },
    ...
    T evaluate(const Feature& feature, T finalDefaultValue) const {
        const expression::EvaluationResult result =
            expression->evaluate(expression::EvaluationContext(&feature));
        if (result) {
