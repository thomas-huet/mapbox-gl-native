...
struct Convert {
    ...
    static std::unique_ptr<Expression> makeZoom() {
        ParsingContext ctx;
        ParseResult zoom =
            createCompoundExpression("zoom", std::vector<std::unique_ptr<Expression>>(), ctx);
        assert(zoom);
        assert(ctx.getErrors().size() == 0);
    ...
    static ParseResult
    makeInterpolate(type::Type type,
        ...
        ParseResult curve = ParseResult(std::make_unique<Interpolate<OutputType>>(
            std::move(type), std::move(interpolator), std::move(input), std::move(convertedStops)));
        assert(curve);
    ...
    static ParseResult makeMatch(type::Type type,
        ...
        for (auto it = stops.begin(); it != stops.end(); it++) {
            ...
            branches.emplace(std::move(key), std::move(it->second));
        }

        return ParseResult(std::make_unique<Match<Key>>(std::move(type), std::move(input),
                                                        std::move(branches),
    ...
    static ParseResult makeCase(type::Type type,
        ...
        std::unique_ptr<Expression> true_case =
            it == stops.end() ? makeError("No matching label") : std::move(it->second);
        ...
        return ParseResult(
            std::make_unique<Case>(std::move(type), std::move(branches), std::move(false_case)));
    }

    template <typename T>
    static ParseResult fromCategoricalStops(std::map<CategoricalValue, T> stops,
                                            const std::string& property) {
        assert(stops.size() > 0);

        std::map<CategoricalValue, std::unique_ptr<Expression>> convertedStops;
        for (const std::pair<CategoricalValue, T>& stop : stops) {
            convertedStops.emplace(stop.first, makeLiteral(stop.second));
        }
        ...
        return firstKey.match(
            ...
            [&](const std::string&) {
                return makeMatch<std::string>(type, makeGet(type::String, property),
                                              std::move(convertedStops));
            },
            [&](int64_t) {
                return makeMatch<int64_t>(type, makeGet(type::Number, property),
                                          std::move(convertedStops));
            });
    }

    template <typename T>
    static std::map<double, std::unique_ptr<Expression>>
    convertStops(const std::map<float, T>& stops) {
        std::map<double, std::unique_ptr<Expression>> convertedStops;
    ...
    static std::unique_ptr<Expression> toExpression(const ExponentialStops<T>& stops) {
        ParseResult e = makeInterpolate<typename ValueConverter<T>::ExpressionType>(
            valueTypeToExpressionType<T>(), makeZoom(), convertStops(stops.stops),
    ...
    static std::unique_ptr<Expression> toExpression(const IntervalStops<T>& stops) {
        ParseResult e(std::make_unique<Step>(valueTypeToExpressionType<T>(), makeZoom(),
    ...
    static std::unique_ptr<Expression> toExpression(const std::string& property,
        ...
        ParseResult e = makeInterpolate<typename ValueConverter<T>::ExpressionType>(
            valueTypeToExpressionType<T>(), makeGet(type::Number, property),
            convertStops(stops.stops), ExponentialInterpolator(stops.base));
        assert(e);
        return std::move(*e);
    }
    ...
    static std::unique_ptr<Expression> toExpression(const std::string& property,
        ...
        ParseResult e(std::make_unique<Step>(valueTypeToExpressionType<T>(), std::move(get),
                                             convertStops(stops.stops)));
        assert(e);
        return std::move(*e);
    }
    ...
    template <typename T>
    static typename std::enable_if_t<util::Interpolatable<T>::value, ParseResult>
    makeZoomCurve(std::map<double, std::unique_ptr<Expression>> stops) {
        return makeInterpolate<typename ValueConverter<T>::ExpressionType>(
            valueTypeToExpressionType<T>(), makeZoom(), std::move(stops),
    ...
    makeZoomCurve(std::map<double, std::unique_ptr<Expression>> stops) {
        return ParseResult(
            std::make_unique<Step>(valueTypeToExpressionType<T>(), makeZoom(), std::move(stops)));
    }
    ...
    static std::unique_ptr<Expression> toExpression(const std::string& property,
        ...
        for (const std::pair<float, std::map<float, T>>& stop : stops.stops) {
            std::unique_ptr<Expression> get = makeGet(type::Number, property);
            ParseResult innerInterpolate =
                makeInterpolate<typename ValueConverter<T>::ExpressionType>(
                    valueTypeToExpressionType<T>(), std::move(get), convertStops(stop.second),
                    ExponentialInterpolator(stops.base));
    ...
    static std::unique_ptr<Expression> toExpression(const std::string& property,
        ...
        for (const std::pair<float, std::map<float, T>>& stop : stops.stops) {
            std::unique_ptr<Expression> get = makeGet(type::Number, property);
            ParseResult innerInterpolate(std::make_unique<Step>(
                valueTypeToExpressionType<T>(), std::move(get), convertStops(stop.second)));
            assert(innerInterpolate);
            outerStops.emplace(stop.first, std::move(*innerInterpolate));
        }
    ...
    static std::unique_ptr<Expression> fromIdentityFunction(type::Type type,
                                                            const std::string& property) {
        std::unique_ptr<Expression> input = type.match(
            [&](const type::StringType&) { return makeGet(type::String, property); },
            [&](const type::NumberType&) { return makeGet(type::Number, property); },
            [&](const type::BooleanType&) { return makeGet(type::Boolean, property); },
            ...
            [&](const auto&) -> std::unique_ptr<Expression> { return makeLiteral(Null); });
