...
public:
    using Stops = std::conditional_t<util::Interpolatable<T>::value,
                                     variant<ExponentialStops<T>, IntervalStops<T>>,
                                     variant<IntervalStops<T>>>;
    ...
    CameraFunction(const Stops& stops)
        : isExpression(false),
          expression(
              stops.match([&](const auto& s) { return expression::Convert::toExpression(s); })),
          zoomCurve(expression::findZoomCurveChecked(expression.get())) {
    }

    T evaluate(float zoom) const {
        const expression::EvaluationResult result =
            expression->evaluate(expression::EvaluationContext(zoom, nullptr));
        if (result) {
    ...
    friend bool operator==(const CameraFunction& lhs, const CameraFunction& rhs) {
