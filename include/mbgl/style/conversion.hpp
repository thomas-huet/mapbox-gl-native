...
/*
   The `conversion` namespace defines conversions from JSON structures conforming to the schema
   defined by
   ...
   `Convertible` is a type that encapsulates a special form of polymorphism over various underlying
   types that
   can serve as input to the conversion algorithm. For instance, on macOS, we need to support
   conversion from both RapidJSON types, and a JSON structure represented with
   ...
   Therefore, we're using a custom implementation of runtime polymorphism where we manually create
   and
   dispatch through a table of function pointers (vtable), while keeping the storage for any of the
   possible
   underlying types inline on the stack, using `std::aligned_storage`.

   For a given underlying type T, an explicit specialization of `ConversionTraits<T>` must be
   provided. This
   ...
   In addition, the type T must be move-constructable. And finally, `Convertible::Storage`, a
   typedef for
   ...
   `Convertible` itself is movable, but not copyable. A moved-from `Convertible` is in an invalid
   state;
...
public:
    ...
    Convertible(T&& value) : vtable(vtableForType<std::decay_t<T>>()) {
        static_assert(sizeof(Storage) >= sizeof(std::decay_t<T>),
    ...
    friend inline optional<Error>
    eachMember(const Convertible& v,
               const std::function<optional<Error>(const std::string&, const Convertible&)>& fn) {
        assert(v.vtable);
...
private:
    ...
    struct VTable {
        ...
        optional<Error> (*eachMember)(
            const Storage&,
            const std::function<optional<Error>(const std::string&, const Convertible&)>&);
    ...
    static auto vtableEachMember(
        const Storage& s,
        const std::function<optional<Error>(const std::string&, const Convertible&)>& fn) {
        return ConversionTraits<T>::eachMember(
            reinterpret_cast<const T&>(s),
            [&](const std::string& k, T&& v) { return fn(k, Convertible(std::move(v))); });
    }
