...
class DatabaseImpl {
public:
    DatabaseImpl(sqlite3* db_) : db(db_) {
    }
...
mapbox::util::variant<Database, Exception> Database::tryOpen(const std::string& filename,
                                                             int flags) {
    sqlite3* db = nullptr;
...
Database::Database(std::unique_ptr<DatabaseImpl> impl_) : impl(std::move(impl_)) {
}
...
void DatabaseImpl::setBusyTimeout(std::chrono::milliseconds timeout) {
    const int err =
        sqlite3_busy_timeout(db, int(std::min<std::chrono::milliseconds::rep>(
...
void Query::bind(int offset,
                 std::chrono::time_point<std::chrono::system_clock, std::chrono::seconds> value) {
    assert(stmt.impl);
    stmt.impl->check(
        sqlite3_bind_int64(stmt.impl->stmt, offset, std::chrono::system_clock::to_time_t(value)));
}
...
std::vector<uint8_t> Query::get(int offset) {
    assert(stmt.impl);
    const auto* begin =
        reinterpret_cast<const uint8_t*>(sqlite3_column_blob(stmt.impl->stmt, offset));
...
template <>
optional<int64_t> Query::get(int offset) {
    assert(stmt.impl);
...
template <>
optional<double> Query::get(int offset) {
    assert(stmt.impl);
    if (sqlite3_column_type(stmt.impl->stmt, offset) == SQLITE_NULL) {
...
template <>
optional<std::string> Query::get(int offset) {
    assert(stmt.impl);
    if (sqlite3_column_type(stmt.impl->stmt, offset) == SQLITE_NULL) {
...
Transaction::Transaction(Database& db_, Mode mode) : dbImpl(*db_.impl) {
    switch (mode) {
    case Deferred:
